...
export const generatePipes = () => {
    ...
    let topPipeHeight = randomBetween(200, (Constants.MAX_HEIGHT / 2) - 200);
...
const Physics = (entities, {touches, time, dispatch}) => {
    ...
    touches.filter(t => t.type === "press").forEach(t => {
        ...
        if (!hadTouches) {
            ...
            const newY = bird.position.y >= 40 ? -10 : -1;
    ...
    Object.keys(entities).forEach(key => {
        ...
        } else if (key.indexOf("floor") === 0 && worldMove) {
        ...
        } else if (key.indexOf("head") === 0 && worldMove) {
            Matter.Body.setPosition(entities[key].body, {
                x: entities[key].body.position.x - speed,
                y: entities[key].body.position.y
            });
        }
    })
